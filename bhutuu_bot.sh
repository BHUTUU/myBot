...
while true; do
	...
	#echo $resp | jq -r .result[0].message.chat.id
	if [ "$?" != '0' ]; then
		exit 1
	fi
...
#		if admeme; then
	...
	"/help"|"/help@bhutuu_bot")
		...
		;;
	"/id"|"/id@bhutuu_bot")
	sendmsg "<code>$fromid</code>"
		;;
	"/def"|"/define"|"/def@bhutuu_bot"|"/define@bhutuu_bot")
		arg || continue
		out=$(exec python3 urban.py "${args[*]}" | jq -r .list[].definition)
		sendmsg "<code>$out</code>"
		;;
	esac
	case "${msg,,}" in
